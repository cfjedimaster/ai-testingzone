...
const si = `
You identify songs based on lyrics. The lyrics may not be perfect. You should only return the identified song and artist, nothing more.
`;
...
async function callGemini(lyrics) {
...
async function handler(req, res) {
	...
	if(req.method === 'GET' && req.url.indexOf('favicon.ico') === -1) {
		...
		res.end();
	} else if(req.method === 'POST' && req.url === '/identify') {
		...
		req.on('end', async () => {
			...
			try {
				result = await callGemini(body.lyrics);
			} catch(e) {
